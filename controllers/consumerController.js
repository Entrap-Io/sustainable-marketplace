...
export const dashboard = async (req, res) => {
  ...
  try {
    ...
    let query = `
      ...
      WHERE u.district = ? AND p.expiration_date >= CURDATE() AND p.stock > 0
    `;
    let queryParams = [userDistrict];
    ...
    const [countResult] = await db.query(countQuery, [userDistrict]);
    console.log(countResult);

    countResult.forEach(e => {
      console.log(e);
    })
