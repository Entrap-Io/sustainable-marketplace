...
export const dashboard = async (req, res) => {
  try {
    const [products] = await db.query(
      ...
      AS is_expired, DATEDIFF(expiration_date, CURDATE()) 
      AS days_left FROM products 
        WHERE market_id = ? ORDER BY id DESC",
      [req.session.user.id]
    );
...
export const updateProfile = async (req, res) => {
  ...
  try {
    await db.query("UPDATE users 
      ...
      WHERE id=?", [name, city, district, req.session.user.id]);
  ...
  try {
    if (req.file) {
      await db.query(
        ...
        stock=?, normal_price=?, discounted_price=?, expiration_date=?, image_filename=? 
        WHERE id=? AND market_id=?",
